...
public class Foundation extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity__foundation);
		 ...
		 ImageView  back=(ImageView) findViewById(id.imageView1);//返回界面
		 TextView	foundation=(TextView) findViewById(id.foundation);//增加信息
		 ...
		 arrayList114=new ArrayList<String>();
	      ...
	      //接收传过来的token值
			...
			System.out.println("桩基界面收到的登录密码"+password);
			 ...
			 System.out.println("桩基界面收到的所属单位"+company);
			 //接收传过来的职务
...
//	 		intent.putExtra("user3", arrayList114);
	 		...
	 		finish();
	 		 Toast.makeText(Foundation.this, "保存到本地成功", Toast.LENGTH_SHORT).show();
	 		}
	 	});
		 ...
		 result.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					intent.putExtra("one", arrayList2);
	   				...
	   				intent.putExtra("four", arrayList5);
	   				intent.putExtra("five", arrayList6);
	   				intent.putExtra("senven", arrayList);
	   				intent.putExtra("eight", arrayList9);
	   				intent.putExtra("nine", arrayList10);
	   				intent.putExtra("ten", arrayList11);
	   				intent.putExtra("token", string);
				...
				public void onClick(View arg0) {
					...
					saveimage.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							if(two.equals("")){
						    	   Toast.makeText(Foundation.this, "电话号码不能为空，请输入", Toast.LENGTH_SHORT).show();
						       }
							//添加姓名
						       String one=name.getText().toString(); int four=name.getText().length();
							if(one.equals("")){
						          Toast.makeText(Foundation.this, "姓名不能为空，请输入", Toast.LENGTH_SHORT).show();
							}else if(four>4){
								Toast.makeText(Foundation.this, "姓名长度最长为4位，请重新输入", Toast.LENGTH_SHORT).show();
							}
							
							//判断输入的值是否符合保存的要求
							if(!two.equals("")&&!one.equals("")&&four<5){
								if(arrayList111.size()>10){
									Toast.makeText(Foundation.this, "名额已满，请选择修改其他用户", Toast.LENGTH_SHORT).show();
						...
						public void onClick(View arg0) {
							...
							if(!two.equals("")&&!one.equals("")&&four<5){
								if(arrayList112.size()>10){
									Toast.makeText(Foundation.this, "最多只能修改三次，请选择修改其他用户", Toast.LENGTH_SHORT).show();
